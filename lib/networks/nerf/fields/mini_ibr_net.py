...
class MiniIBRNet(nn.Module):
    ...
    def forward(self, xyz_encoding, dir_encoding):
        ...
        rgb = img_feat_rgb_dir[..., :3]
        weight = img_feat_rgb_dir[..., 3:4]
        weight = F.softmax(F.softplus(weight-1.), dim=1)
        color = (weight * rgb).sum(dim=1)
        return color, None
