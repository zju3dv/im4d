...
def get_bound_2d_mask_padding(bounds, K, pose, H, W, padding=0.1):
    bounds = bounds.copy()
    bounds[:1] -= padding
    bounds[1:] += padding
    corners_3d = get_bound_corners(bounds)
...
def get_sample(input):
    if len(input) == 4 and input[-1] == -1:
        return np.arange(input[0], input[1])[::input[2]].tolist()
    elif -1 in input and len(input==2):
        return input[:1]
    elif -1 not in input:
        return input
    else:
...
def get_xywh_tensor(mask):
    xywh = []
    for b in range(len(mask)):
        nonzero = mask[b].nonzero()
        x1, y1, x2, y2 = nonzero[:, 1].min(), nonzero[:, 0].min(), nonzero[:, 1].max(), nonzero[:, 0].max()
        xywh.append([x1, y1, x2-x1+1, y2-y1+1])
    return torch.as_tensor(xywh)