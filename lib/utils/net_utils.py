...
def load_model(net,
               ...
               strict=True):
    if not resume:
        print(colored('remove contents of model directory %s' % model_dir, 'red'))
        assert(model_dir != '/')
        os.system('rm -rf {}'.format(os.path.join(model_dir, '*')))
        return 0
    ...
    if 'optim' in pretrained_model:
        try:
            ...
            if cfg.bkgd_render:
                print('Warning: optimizer is not loaded, but okay.')
                return pretrained_model['epoch'] + 1
            else:
                print('Warning: optimizer is not loaded, but okay.')
                return pretrained_model['epoch'] + 1
                print('Error: optimizer is not loaded, not okay.')
                sys.exit(1)
    else:
        return 0
...
def save_model(net, optim, scheduler, recorder, model_dir, epoch, last=False):
    ...
    if last:
        torch.save(model_last, os.path.join(model_dir, 'latest.pth'))
    else:
...
def load_network(net, model_dir, resume=True, epoch=-1, strict=True):
    ...
    if not os.path.exists(model_dir):
        print(colored('pretrained model does not exist: {}'.format(model_dir), 'red'))
        return 0
    ...
    pretrain_net = pretrained_model['net']
    # pretrain_net = {k:pretrain_net[k] for k in pretrain_net if 'rendering_head' not in k}
    net.load_state_dict(pretrain_net, strict=False)
...
def save_trained_config(cfg, split='train'):
    train_config_path = os.path.join(cfg.result_dir, '{}_config_{}.yaml'.format(split, os.popen('git describe --tags --always').readline().strip()))
    open(train_config_path, 'w').write(cfg.dump())

def load_pretrain_head(net, model):
    model_path = os.path.join(cfg.workspace, 'pretrain', model)
    print('Load pretrain model: {}'.format(model_path))
    model = torch.load(model_path, 'cpu')
    if 'finetune' in model_path:
        feature_net = {'.'.join(k.split('.')[1:]):model['net'][k] for k in model['net'].keys() if 'feature_net' in k}
        net.feature_net.load_state_dict(feature_net)
        rendering_head = {'.'.join(k.split('.')[2:]):model['net'][k] for k in model['net'].keys() if 'rendering_head' in k}
        net.implicit_decoder.rendering_head.load_state_dict(rendering_head)
        return 0
    elif 'cnn_ibr' in model_path:
        feature_net = {'.'.join(k.split('.')[1:]):model['net'][k] for k in model['net'].keys() if 'feature_net' in k}
        net.feature_net.load_state_dict(feature_net)
        rendering_head = {'.'.join(k.split('.')[1:]):model['net'][k] for k in model['net'].keys() if 'nerf.agg' in k or 'nerf.color' in k}
        net.implicit_decoder.rendering_head.load_state_dict(rendering_head, strict=True)
        return 0
    feature_net = {'.'.join(k.split('.')[1:]):model['net'][k] for k in model['net'].keys() if 'feature_net' in k}
    net.feature_net.load_state_dict(feature_net)

    rendering_head = {'.'.join(k.split('.')[1:]):model['net'][k] for k in model['net'].keys() if 'nerf_1.agg' in k or 'nerf_1.color' in k}
    rendering_head.update({ '.'.join(k.split('.')[1:]).replace('lr1', 'lr0'):model['net'][k] for k in model['net'].keys() if 'nerf_1.lr1' in k})
    net.implicit_decoder.rendering_head.load_state_dict(rendering_head, strict=True)
    return 0


def load_pretrain(net, model_dir):
    model_dir = os.path.join(cfg.workspace, 'trained_model', cfg.task, cfg.scene, model_dir)
    if not os.path.exists(model_dir):
        return 1
